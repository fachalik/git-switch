...
pub fn render_include_file(profile: &Profile) -> Result<String> {
    ...
    //
    // git hands this value to `sh -c` whenever it holds a shell metacharacter,
    // and ours always holds spaces — so the shell is a certainty and the
    // quoting is the only thing keeping a key path from becoming a command.
    // Two layers, innermost first: sh_quote makes the path one shell word,
    // git_quote makes the whole line one config value. git_quote escapes only
    // `\` and `"`, so the single quotes pass through its parser untouched.
    //
    // The expanded absolute path is emitted rather than the `~` form, so no
    // tilde-expansion behaviour (shell vs ssh vs libgit2) is load-bearing.
    let key = paths::expand_within_home(&profile.ssh_key_path)?;
    let key = key
        .to_str()
        .ok_or_else(|| AppError::validation("key path contains unreadable characters"))?;
    out.push_str(&format!(
        "\tsshCommand = {}\n",
...
mod tests {
    ...
    fn include_file_quotes_values() {
        ...
        let rendered = render_include_file(&p).unwrap();
    ...
    #[test]
    fn sshcommand_single_quotes_the_key_path() {
        let mut p = profile("work", &[]);
        p.ssh_key_path = "~/My Keys/id_work".into();
        let rendered = render_include_file(&p).unwrap();

        let home = paths::home().unwrap();
        let expected = format!(
            "\tsshCommand = \"ssh -i '{}/My Keys/id_work' -o IdentitiesOnly=yes\"\n",
            home.display()
        );
        assert!(
    ...
    #[test]
    fn sshcommand_never_lets_a_key_path_become_a_command() {
        // The payload that motivated the quoting. It cannot be stored through
        // validate_ssh_key_path, but render must not depend on that: store.rs
        // does not revalidate, so a hand-edited profiles.json reaches here.
        let mut p = profile("work", &[]);
        p.ssh_key_path = "~/.ssh/id;touch /tmp/pwned".into();
        let rendered = render_include_file(&p).unwrap();

        let line = rendered
            .lines()
            .find(|l| l.contains("sshCommand"))
            .expect("sshCommand line");
        // The whole payload sits inside one single-quoted shell word: exactly
        // two quotes, so nothing escapes to be read as a second command.
        assert_eq!(line.matches('\'').count(), 2, "in {line:?}");
        let start = line.find('\'').unwrap();
        let end = line.rfind('\'').unwrap();
        assert!(
            line[start..end].contains(";touch /tmp/pwned"),
            "payload must stay inside the quoted run: {line:?}"
        );
    }
